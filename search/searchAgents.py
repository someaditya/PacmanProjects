...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        self.startState = (self.startingPosition[0],self.startingPosition[1],tuple(cornerState))
    ...
    def isGoalState(self, state):
        ...
        for corner in state[2]:
            ifDotExists = corner[1]
            if ifDotExists is True:
                return False
        return True   
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                nextState = (nextx, nexty)
                #nextState is a tuple
                newGoalState = []
                for corner in goalState:
                    cornerCoordinate = corner[0]
                    hasDot = corner[1]
                    if nextState == cornerCoordinate:
                        newGoalState.append((cornerCoordinate, False))
                    else:
                        newGoalState.append((cornerCoordinate, hasDot))
...
def cornersHeuristic(state, problem):
    ...
    for corner in visited:
        ...
        cornersVisited = dotCoordinates
        currentPosition = state[0], state[1]
        
        distance = abs(cornersVisited[0] - currentPosition[0]) + abs(cornersVisited[1] - currentPosition[1])
        
        if distance > heuristic:
            heuristic = distance 

    if  cornerDots == len(corners):
        return 0
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        frontier.push((initial, actionlist))
